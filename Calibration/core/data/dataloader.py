...
from core.data.dataset import GeneralDataset, ViT_Dataset
...
ViTclassifyBatch = namedtuple(
    'ViTclassifyBatch', ['pixel_values', 'labels'])
def get_vit_dataloader(config, mode, feature_extractor):
    """Get the dataloader corresponding to the model type and training phase.
    ...
    # trfms_list.append(transforms.ToTensor())
    # trfms_list.append(transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)))
    # trfms = transforms.Compose(trfms_list)
    ...
    def vit_collate_function(raw_batch):

        pixel_values = feature_extractor(
            images=[example[0] for example in raw_batch],
