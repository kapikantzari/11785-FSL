...
class ResNet(nn.Module):
   ...
   #     self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.layer4 = self._make_layer(block, 512, stride=2, drop_rate=drop_rate, drop_block=drop_block, block_size=dropblock_size, pool=False)
  ...
  #      x = self.bn1(self.relu(self.conv1(x)))
    ...
    def forward_with_drop(self, x, drop_list):

        x = self.layer1(x)


        x = self.layer2(x)*drop_list[0] + x * (1 - drop_list[0])
        x = self.layer3(x)*drop_list[1] + x * (1 - drop_list[1])
        x = self.layer4(x)*drop_list[2] + x * (1 - drop_list[2])
        x = self.avgpool(x)
        #x = x.view(x.size(0), -1)
        #result = self.fc(x)
        return x
