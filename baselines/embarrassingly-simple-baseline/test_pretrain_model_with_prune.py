...
def val_nn_prune(model, loader):
    ...
    for i, batch in enumerate(loader, 1):
        ...
        for pr in range(100):
            ...
            if acc > acc_best:
                acc_best = acc
        print("Best Acc for batch %d: %.4f" % (i, ave_acc.item()))
        ave_acc.add(acc_best)
    print("One Shot Test Acc Best Trial Mean %.4f" % ( ave_acc.item()))
...
with torch.no_grad():
    ...
    val_nn_prune(model1, dataloader)
